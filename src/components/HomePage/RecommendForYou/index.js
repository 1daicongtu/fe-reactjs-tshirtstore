...
const RecommendForYou = () => {
    ...
    const handleChangeCategoryActive = (event, category) => {
        ...
        if (
            refBoxItemCategory.current &&
            event.currentTarget?.tagName === 'LI'
        ) {
            const widthBoxItemCategoryAvg =
                refBoxItemCategory.current.offsetWidth / 2;
    ...
    return listCategories && listCategories.length > 0 ? (
        <div className={clsx(styles.recommendForYou)}>
            <h3 className={clsx(styles.titleRecommendForYou)}>
                Recommend For You
            </h3>
            <ul ref={refBoxItemCategory} className={clsx(styles.listCategory)}>
                <li
                    className={clsx(
                        styles.itemCategory,
                        'All' === categoreActive ? styles.active : '',
                    )}
                    onClick={(e) => {
                        handleChangeCategoryActive(e, 'All');
                    }}
                >
                    All
                </li>
                    ...
                    listCategories.map((category, index) => {
                        return (
                            <li
                                key={index}
                                onClick={(e) => {
                                    handleChangeCategoryActive(e, category);
                                }}
                                className={clsx(
                                    styles.itemCategory,
                                    category === categoreActive
                                        ? styles.active
                                        : '',
                                )}
                            >
