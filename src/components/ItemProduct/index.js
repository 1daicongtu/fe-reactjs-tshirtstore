...
const ItemProduct = ({product}) => {
    ...
    const listWishList = useSelector(state => state.wishlist.wishLists)?.map((product)=>{
        return product.productID;
    });
    const listCompare = useSelector(state => state.compare.compareList)?.map(product => {
        return product.productID;
    }) ;
    const [wishListTick, setWishListTick] = useState(listWishList.includes(product.productID) ? true : false);
    ...
    return (
        ...
        <div className={`col-6 col-md-4 col-lg-3 ${clsx(styles.productItem)}`}>
            <div className={` ${clsx(styles.productItemDetail)}`}>
                <div className={` ${clsx(styles.productImageBox)}`}>

                    <Link to="/" className={clsx(styles.productImageBoxOverlay)}>
                        <img src={productDetailActive?.type?.[0]?.imageToShow?.front} alt="anh1"
                            className={clsx(styles.frontImage)}
                        />
                        <img src={productDetailActive?.type?.[0]?.imageToShow?.back} alt="anh2" 
                            className={clsx(styles.backImage)}
                        />
                    </Link>
                    
                    {product.isSale ? <span className={clsx(styles.saleTitle)}>SALE</span> : ""}
                    <div className={clsx(styles.productAction)}>
                        <button className={clsx(styles.btnActionProduct)}
                            data-tooltip-id={`id-btn-selectOption-${product.productID}}`}
                            data-tooltip-content='Select options' 
                            onClick={()=>{showQuickViewPopup(product)}}
                        >
                            <i className="fa-solid fa-plus"></i>
                        </button>
                        <Tooltip id={`id-btn-selectOption-${product.productID}}`}/>
                        <button className={clsx(styles.btnActionProduct, styles.btnWishlist)}
                            data-tooltip-id={`id-btn-addToWishlist-${product.productID}`}
                            data-tooltip-content='Add to Wishlist' 
                            onClick={()=> {toggleAddRemoveProductToWishList(product)}}
                        >
                            {(listWishList.includes(product.productID)) ? <i className={`fa-solid fa-heart  ${styles.active}`}></i> : <i className="fa-regular fa-heart"></i>}
                        </button>
                        <Tooltip id={`id-btn-addToWishlist-${product.productID}`}/>
                        <button 
                        ...
                        <button className={clsx(styles.btnActionProduct, styles.btnActionCompare)}
                        ...
                        >
                            {
                                ...
                                ? 
                                <i className={`fa-solid fa-check ${styles.active}`}></i> 
                                : 
                ...
                <div className={`${clsx(styles.productInfoDetail)}`}>
                    <p className={clsx(styles.productCategory)}>{product.category.join(", ")}</p>
                    <Link to="/" className={clsx(styles.linkToDetail)}>
                        <p className={clsx(styles.productName)}>{product.productName}</p>
                    </Link>             
                    <p className={clsx(styles.productPrice)}>${productDetailActive?.type?.[0]?.price}.00</p>
                    <ul className={clsx(styles.listProductColor)}>
                        {
                            getColor(product)?.map((color, index)=>{
                                return (
                                    <li key={index} 
                                        className={clsx(styles.itemProductColor, color === productDetailActive.color ? styles.active : "")} 
                    ...
                    <ul className={clsx(styles.listSizeProduct)}>
                        {product && product.size.length > 0 && product.size.map((size, index) => {
                            return <li key={index} className={clsx(styles.itemProductSize, size === sizeSelected ? styles.active : "")}
            ...
            <PopupProductQuickView product={productPopup} show={showPopup} setShowPopup={setShowPopup}></PopupProductQuickView>
        </div>  
