    ...
    const handleUpdateCoupon = ()=>{
        if (inputVoucher.trim() !== ""){
            ...
            listVoucher.forEach(item => {
                if(item.code.toUpperCase() === inputVoucher.trim().toUpperCase()){
                    setVoucherSelected(item);
                    flag = true;
                }
            })
    ...
    return (
        <>
            ...
            <div className={clsx(styles.modalBox, props.cartList ? styles.active : "")}>
                ...
                <div className={clsx(styles.layerModalNote, !windowNote ? styles.unactive : "")}
                    onClick={() => {setWindowNote(false)}}
                ></div>
                <div className={clsx(styles.layerModalShipping, !windowShipping ? styles.unactive : "")}
                    onClick={() => {setWindowShipping(false)}}
                ></div>
                <div className={clsx(styles.layerModalCoupon, !windowCoupon ? styles.unactive : "")}
                    onClick={() => {setWindowCoupon(false)}}
                ></div>

                <div className={clsx(styles.addNote, windowNote ? styles.active : "")}>
                ...
                <div className={clsx(styles.shipping, windowShipping ? styles.active: "")}>
                    <div className={clsx(styles.closeModalShipping)}
                        onClick={() => {setWindowShipping(false)}}
                    >
                        <span>
                            <i class="fa-solid fa-xmark"></i>
                    ...
                    <form action="#">
                        ...
                        <input type="text" name="cityName" className={clsx(styles.inputShipping)} placeholder="City" required/>
                        <input type="text" name="passcodeCity" className={clsx(styles.inputShipping)} placeholder="Passcode / ZIP" required/>

                        <button className={clsx(styles.btnSubmitShipping)}
                ...
                <div className={clsx(styles.coupon, windowCoupon ? styles.active : "")}>
                    ...
                    <div className={clsx(styles.listCoupon)}>
                        ...
                        {listVoucher.map((item, index)=>{
                            return (
                                <div className={clsx(styles.itemCoupon, voucherSelected?.id === item.id ? styles.active : "")} key={item.id}
                                    onClick={() => handleClickVoucher(item)}
                                > 
                                    <p className={clsx(styles.valueCoupon)}>{item.type == "money" ? `$${item.value}` : `${item.value}%`} Discount</p>
                                    <div className={clsx(styles.couponInfo)}>
                                        <button>{item.code}</button>
                                        <span>{item.date === null ? "Never Expire" : `${monthNames[item.date.getMonth()]} ${item.date.getDay()} ${item.date.getFullYear()}` }</span>
                                    </div>
                                    <p className={clsx(styles.descriptCoupon)}>{item.desc}</p>
                                </div>
                            )
                        })}
                    </div>
                    <h1 className={clsx(styles.couponTitle)}>Have a coupon code?</h1>
                    <input type="text" className={clsx(styles.inputVoucher)} value={inputVoucher} onChange={e => setInputVoucher(e.target.value)}/>

                    <button className={styles.btnApplyCoupon}
