...
import {removeItemToCompareList, clearAll} from "../../../redux/slices/compareSlice";
...
const Compare = () => {
    ...
    return (
        <>
            <div className={clsx(styles.modal, !compareList ? clsx(styles.unactive) : "")}
                onClick={() => {
            ...
            <div className={clsx(styles.modalBox, compareList ? clsx(styles.active)  : "")}>
                ...
                <div className={`row g-0 ${clsx(styles.modalBoxBody)}`}>
                    <div className={`col-12 col-md-3 col-lg-3 ${clsx(styles.modalBoxTitle)}`}>
                        <div className={clsx(styles.containerTitle)}>
                            ...
                            <p className={clsx(styles.countProduct)}>({compareStore ? compareStore.length : 0} Products)</p>
                        </div>
                    </div>
                    <div className={`col-12 col-md-6 col-lg-6`}>
                        <div className={clsx(styles.listImage)}>
                                ...
                                compareStore.map((product, index)=>{
                                    return (
                                        <NavLink className={clsx(styles.itemProduct)} to="/">
                                            <div className={clsx(styles.boxImageZoom)}>
                                                <img src={product.listProduct?.[0]?.type?.[0]?.imageSmall?.front} alt={product.productID} 
                                                    data-tooltip-id={product.productID}
                                                    data-tooltip-content={product.productName}
                                                />
                                                <img className={clsx(styles.imageZoom)} src={product.listProduct?.[0]?.type?.[0]?.imageToShow.front} alt={"zoom"} />
                                                <Tooltip id={product.productID} place="bottom"/>
                                            </div>

                                            <span>
                                                <i className="fa-solid fa-xmark"
                                                    onClick={()=>{dispatch(removeItemToCompareList(product.productID))}}
                                                ></i>
                                            </span>
                                        </NavLink> 
                                    )
                            ...
                            {countItemCompareBlank > 0 && 
                                Array(countItemCompareBlank).fill(0).map(()=>{
                                    return (<div className={clsx(styles.haventProduct, clsx(styles.itemProduct))}>
